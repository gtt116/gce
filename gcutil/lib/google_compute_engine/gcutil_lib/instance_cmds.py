...
class AddInstance(InstanceCommand):
  ...
  def __init__(self, name, flag_values):
    ...
    flags.DEFINE_string('image',
                        None,
                        '[Required] Specifies the image to use for this '
  ...
  def Handle(self, *instance_names):
    ...
    if (self._flags.persistent_boot_disk is None
      ...
      self._flags.persistent_boot_disk = self._PresentSafetyPrompt(
          'The boot disk is unspecified.  I can create a new persistent '
          'boot disk and use it (preferred), or use a scratch disk (not '
          'recommended).  Do you want to use a persistent boot disk')
